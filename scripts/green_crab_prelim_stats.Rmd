...
gc_dat_upd <- read.csv("../data/Condo_Data_Raw_CML_WEL_11_10_22.csv")

gc_dat_upd$Color <- factor(gc_dat_upd$Color, 
                           levels = c('1', '2', '3', '4', '5'))
gc_dat_upd$Color <- as.factor(gc_dat_upd$Color)

gc_dat_upd$RI <- as.numeric(gc_dat_upd$RI)
...
boxplot(RI ~ Color, data = gc_dat_upd,
         main = "Hemolymph Protein vs. Color in Green Crabs", 
         xlab = "Hemolymph Color (on a scale of 1-5)", 
         ylab = "Protein Concentration", 
         col= c('white', 'aliceblue', 'bisque', 'coral', 'brown3'))
```
```{r updated ri vs color graph}
...
```{r Tukey HSD test for color and ri Updated}

fcol <- factor(gc_dat_upd$Color)
# color is on a scale of 1-5; appears as "numeric"; categorical factor

ri_col_aov_upd <- aov (RI ~ fcol, data=gc_dat_upd)
summary(ri_col_aov_upd)
# type 1 aov used for Tukey test
# tests between categories: is 1 dif. than 2, etc.

TukeyHSD(ri_col_aov_upd, conf.level = 0.95) 
