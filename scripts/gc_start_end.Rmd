...
se_dat_upd <- read.csv("../data/se_updated_final.csv")

# define levels as factors for later graphing

se_dat_upd$hemo_ri <- as.numeric("../data/se_dat_upd$hemo_ri")
...
```{r Initial Hemo Protein vs. outcome}

boxplot(hemo_ri~outcome, data=subset_df,
        xlab = NULL, ylab = 'Hemolymph Refractive Index',
        col= c('steelblue4', 'plum'))

t.test(hemo_ri ~ outcome, data=subset_df)
...
```{r Initial Hemolymph RI vs. Color}

# Box plot looking at hemo color vs. RI in only initial 
# crabs. 
